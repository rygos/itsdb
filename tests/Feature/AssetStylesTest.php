...
class AssetStylesTest extends TestCase
{
    ...
    public function test_server_view_contains_compose_workspace_markup(): void
    {
        ...
        $this->assertStringContainsString('data-product-checkbox="', $view);
    ...
    public function test_server_view_contains_compose_workspace_script(): void
    {
        ...
        $this->assertStringContainsString('function toggleProductSelection(productId) {', $view);
        $this->assertStringContainsString('window.toggleComposeWorkspaceProduct = toggleProductSelection;', $view);
        ...
        $this->assertStringContainsString("console.debug('compose-workspace summary', {", $view);
        $this->assertStringContainsString('function normalizeId(value) {', $view);
        $this->assertStringContainsString('productMap[normalizeId(product.id)]', $view);
        $this->assertStringContainsString('function getProductDisplayLabel(product) {', $view);
        $this->assertStringContainsString('function getRenderedProducts() {', $view);
        $this->assertStringContainsString('function getRenderedProductMap() {', $view);
        $this->assertStringContainsString('function getSelectedProductIdsFromDom() {', $view);
        $this->assertStringContainsString('function getSelectedProducts(selectedProductIds) {', $view);
        $this->assertStringContainsString("Array.from(root.querySelectorAll('[data-product-item].is-selected'))", $view);
        $this->assertStringContainsString('return fallbackParts.join(\' / \');', $view);
        $this->assertStringContainsString('var selectedProductIds = getSelectedProductIdsFromDom();', $view);
        $this->assertStringContainsString('var selectedProducts = getSelectedProducts(selectedProductIds);', $view);
        $this->assertStringContainsString('var addedContainerIds = uniqueValues(', $view);
        ...
        $this->assertStringContainsString('productContainerIds[productId].add(normalizeId(container.id));', $view);
        $this->assertStringContainsString('containerProductIds[containerId].add(normalizeId(product.id));', $view);
        $this->assertStringContainsString('if (!container) {', $view);
        $this->assertStringContainsString("item.classList.toggle('is-selected', !isSelected);", $view);
        $this->assertStringContainsString("item.setAttribute('data-product-selected', selectedProductIds.has(productId) ? 'true' : 'false');", $view);
    ...
    public function test_server_compose_workspace_styles_exist(): void
    {
        ...
        $this->assertStringContainsString('.server-compose-picker__checkbox {', $styles);
